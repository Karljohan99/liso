...
class BoxLearner(torch.nn.Module):
    def __init__(self, cfg) -> None:
        super().__init__()
        self.cfg = cfg
        self.activations = {k: allowed_activations[v] for k, v in self.cfg.box_prediction.activations.items()}
        ...
        pred_bev_maps_shape = get_centermaps_output_grid_size(self.cfg, np.array(self.cfg.data.img_grid_size))
        if pred_bev_maps_shape is not None:
            ...
            self.pillar_center_coors_m = torch.nn.parameter.Parameter(
                torch.from_numpy(get_metric_voxel_center_coords(bev_range_x=self.cfg.data.bev_range_m[0], bev_range_y=self.cfg.data.bev_range_m[1], 
    ...
    def forward(self, img_t0, pcls_t0, gt_boxes=None, centermaps_gt=None, train=True):# -> Tuple[Shape, Dict]:
        ...
        if img_t0 is not None:
            assert (len(img_t0.shape) == 4), f"need batched gray image but got shape {img_t0.shape}"
    ...
    def apply_all_output_modifications(self, *, raw_box_vars: Dict[str, torch.FloatTensor], gt_boxes=None, centermaps_gt=None):
        ...
        need_gt_replacement = np.any(["gt" in v for _, v in self.cfg.box_prediction.output_modification.items()])
...
def select_network(cfg, device):
    if cfg.network.name in (
        ...
        box_predictor = BoxLearner(cfg).to(device)
