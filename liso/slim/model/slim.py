...
class SLIM(nn.Module):  # type:ignore
    def __init__(self, cfg, num_train_samples, *args, **kwargs):
        ...
        self.head_decoder_fw = HeadDecoder(self.slim_cfg, bev_extent=bev_pc_range, name="head_decoder_forward")
        self.head_decoder_bw = HeadDecoder(self.slim_cfg, bev_extent=bev_pc_range, name="head_decoder_backward")

        assert self.slim_cfg.phases.train.mode in ["supervised", "unsupervised"], self.config.mode
        ...
        self.moving_dynamicness_threshold = MovingAverageThreshold(num_train_samples=num_train_samples, 
                                                                   num_moving=621013971,  # TODO: where did this number come from?
                                                                   num_still=num_still_points)
        
        self.raft_network = RAFT(cfg=self.cfg, head_decoder_fw=self.head_decoder_fw, head_decoder_bw=self.head_decoder_bw)

    def forward(self, sample_data_t0, sample_data_t1, summaries):

        # predictions is list across iterations
        outputs_fw, outputs_bw, aux_outputs = self.raft_network(get_network_input_pcls(self.cfg, sample_data_t0, "ta", to_device="cuda"),
                                                                get_network_input_pcls(self.cfg, sample_data_t1, "ta", to_device="cuda"))
        
        filled_pillar_mask_t0 = torch.squeeze(aux_outputs["t0"]["bev_net_input_dbg"] > 0.5, dim=1)
        filled_pillar_mask_t1 = torch.squeeze(aux_outputs["t1"]["bev_net_input_dbg"] > 0.5, dim=1)
        ...
        for it, (net_output_0_1, net_output_1_0) in enumerate(zip(outputs_fw, outputs_bw)):
            # Summaries
            should_write_img_summaries = summaries["imgs_eval"] and (it == len(outputs_fw) - 1)
            should_write_metric_summaries = summaries["metrics_eval"] and (it == len(outputs_fw) - 1)
            cur_summaries = {"writer": summaries["writer"],
            ...
            prediction_fw = self.head_decoder_fw(
                ...
                pointwise_valid_mask=sample_data_t0["pcl_ta"]["pcl_is_valid"].to(net_output_0_1.device),
                pointwise_voxel_coordinates=sample_data_t0["pcl_ta"]["pillar_coors"].to(net_output_0_1.device),
                ...
                ohe_gt_stat_dyn_ground_label_bev_map=sample_data_t0.get("ohe_gt_stat_dyn_ground_label_bev_map_t0", None),
            ...
            prediction_bw = self.head_decoder_bw(
                net_output_1_0,
                dynamicness_threshold=self.moving_dynamicness_threshold.value(),
                pointwise_valid_mask=sample_data_t1["pcl_ta"]["pcl_is_valid"].to(net_output_1_0.device),
                pointwise_voxel_coordinates=sample_data_t1["pcl_ta"]["pillar_coors"].to(net_output_1_0.device),
                ...
                summaries=cur_summaries,
                ohe_gt_stat_dyn_ground_label_bev_map=sample_data_t1.get("ohe_gt_stat_dyn_ground_label_bev_map_t1", None),
                dynamic_flow_is_non_rigid_flow=self.slim_cfg.model.dynamic_flow_is_non_rigid_flow,
            )

            not_enough_points += (prediction_fw["not_enough_points"].detach().cpu().numpy().sum())
            not_enough_points += (prediction_bw["not_enough_points"].detach().cpu().numpy().sum())
